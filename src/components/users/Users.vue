    ...
    >
      ...
      >
        ...
        <el-form-item label="地址" prop="path">
          <el-input v-model="addUserForm.path"></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    addUser() {
      this.$refs.addUserFormRef.validate(async valid => {
        if (!valid) return;
        const { data } = await this.$axios.post("/users", this.addUserForm,{
          headers: {
            "Authorization": localStorage.getItem("token")
          }});
        if (data.code !== 200) {
    ...
    editUser() {
      this.$refs.editUserFormRef.validate(async valid => {
        if (!valid) return;
        const { data } = await this.$axios.put(`users/${this.editUserForm.id}`, {
    ...
    /*  console.log(this.selectRoleId)*/
      const {data} = await this.$axios.put(`users/${this.userInfo.id}/role/${this.selectRoleId}`,{
